...
class TestsCleepbus(unittest.TestCase):
    ...
    def test_decode_peer_infos(self):
        ...
        self.assertDictEqual(peer_infos.to_dict(True), {
            'ssl': False,
            'port': 1666,
            'macs': ['00:00:00:00:00', '11:11:11:11:11'],
            'hostname': 'dummy',
    ...
    def test_get_peers(self):
        self.init_session()
        self.module.peers = {
            'peer1': PeerInfos(uuid='123', ident='666', cleepdesktop=False, port=8000, macs=['00:00:00:00:00']),
            'peer2': PeerInfos(uuid='456', ident='999', cleepdesktop=True, hostname='dummy'),
        }

        peers = self.module.get_peers()

        self.assertDictEqual(peers, {
            'peer1': {
                'cleepdesktop': False,
                'uuid': '123',
                'ident': '666',
    ...
    def test_on_peer_connected(self):
        ...
        self.assertDictEqual(self.module.peers['123-456-789'].to_dict(True), {
            'uuid': '123-456-789',
            'ident': '987-654-321',
            'ip': '127.0.0.1',
            'port': 80,
    ...
    def test_send_command_to_peer_check_parameters(self):
        ...
        with self.assertRaises(MissingParameter) as cm:
            self.module._send_command_to_peer(None, 'dummy', peer_infos.uuid, {'param1': 'value1'})
        self.assertEqual(str(cm.exception), 'Parameter "command" is missing')
        with self.assertRaises(MissingParameter) as cm:
            self.module._send_command_to_peer('my_command', None, peer_infos.uuid, {'param1': 'value1'})
        self.assertEqual(str(cm.exception), 'Parameter "to" is missing')
        with self.assertRaises(MissingParameter) as cm:
            self.module._send_command_to_peer('my_command', 'dummy', None, {'param1': 'value1'})
...
class TestsPyrebus(unittest.TestCase):
    ...
    GET_IFADDRS = [
        {
            'ens37': {
                ...
                2: {'addr': '192.168.40.128', 'netmask': '255.255.255.0', 'broadcast': '192.168.40.255'},
                10: {'addr': 'fe80::136a:cf3d:9c98:2ffc', 'scope': 2, 'netmask': 'ffff:ffff:ffff:ffff::'}
            }
        }, {
            'ens37no17': {
                2: {'addr': '192.168.40.128', 'netmask': '255.255.255.0', 'broadcast': '192.168.40.255'},
            ...
            'br-858bcb464b98': {
                17: {'addr': '02:42:72:50:50:7e'}
            }
        }, {
            'docker0': {
                17: {'addr': '02:42:3e:2f:05:f6'}
            }
        }, {
            'veth30b3b80': {
                17: {'addr': 'c6:28:75:af:c6:f1'},
                10: {'addr': 'fe80::c428:75ff:feaf:c6f1', 'scope': 6, 'netmask': 'ffff:ffff:ffff:ffff::'}
            }
        }, {
            'lo': {
                17: {'addr': '00:0c:29:20:7d:5f'},
            ...
            'eth1': {
                ...
                2: {'addr': '82.64.200.227', 'netmask': '255.255.255.0', 'broadcast': '82.64.200.255'},
                10: {'addr': 'fe80::5604:a6ff:fecc:b3b6', 'scope': 2, 'netmask': 'ffff:ffff:ffff:ffff::'}
            }
        }
    ]
    ...
    def setUp(self):
        ...
        self.peer_infos= PeerInfos(uuid='123-456-789', ident='987-654-321', ip='1.2.3.4')
    ...
    def test_start_check_parameters(self):
        ...
        with self.assertRaises(Exception) as cm:
            self.lib.start(None)
        self.assertEqual(str(cm.exception), 'Parameter "infos" is not specified or invalid')
        with self.assertRaises(Exception) as cm:
            self.lib.start({})
        self.assertEqual(str(cm.exception), 'Parameter "infos" is not specified or invalid')
        with self.assertRaises(Exception) as cm:
        ...
        self.assertEqual(str(cm.exception), 'Parameter "bus_name" is not specified or invalid')
        with self.assertRaises(Exception) as cm:
            self.lib.start({'field': 'value'}, 12.34)
        self.assertEqual(str(cm.exception), 'Parameter "bus_name" is not specified or invalid')
        with self.assertRaises(Exception) as cm:
            self.lib.start({'field': 'value'}, 'bus', '')
        self.assertEqual(str(cm.exception), 'Parameter "bus_channel" is not specified or invalid')
        with self.assertRaises(Exception) as cm:
    ...
    def test_message_to_receive_from_pipe_shout(self):
        ...
        self.assertDictEqual(self.messages[0]['message'].to_dict(), {
            'broadcast': ANY,
    ...
    def test_send_message(self):
        ...
        mock_pipein.send.assert_called_with(json.dumps(message.to_dict()).encode('utf8'))
